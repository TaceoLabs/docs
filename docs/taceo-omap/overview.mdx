...
TACEO:OMap can be seen as a **private version of this** `mapping`. It realizes [private shared state](https://core.taceo.io/articles/private-shared-state/) using secret sharing and MPC, **supporting confidential reads and writes** where no single node ever sees plaintext values. Crucially, the access pattern is also hidden - no node learns *which* key is being read or written, only that a valid operation occurred.
