...
Private payments on the EVM chains you already use, built on Merces. Merces wraps ERC-20s like USDC into shielded balances, with compliance primitives at the protocol layer.

**Live on Arc, Base, and Plasma testnets.** ~5M demo transactions, ~300 TPS, single-digit-cents gas on L2. Mainnet deployment in progress. The MPC protocol underneath is the same one TACEO co-architected for World's iris-code system, in production at global scale.
