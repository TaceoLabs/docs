...
If one or more of the selected node operators is offline or unresponsive, the call may fail. In that case, you’ll need to:
...
> ⚠️ Important: If you schedule redundant jobs, you **must perform secret-sharing separately for each**. Even if some node operators overlap across jobs, this prevents any leakage and maintains security. If you are using our client libraries, this is done for you.
