...
Once a user is deemed eligible, the blueprint creator authenticates with the <span className="tooltip" data-tooltip="Cryptographic Coordination Layer">CCL</span> using their credentials. The CCL then issues a signed voucher, which is returned to the user.
