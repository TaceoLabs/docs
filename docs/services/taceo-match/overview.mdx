...
Each MPC node decrypts its share and participates in the match protocol against the database. The protocol is parameterised by the distance function - for example, **Hamming distance** for iris codes (see [The Matching Function](/docs/services/taceo-match/concepts#the-matching-function)). Only the final verdict is revealed: match or no match.
...
- Understand the cryptographic foundations? Read the [Concepts](/docs/services/taceo-match/concepts) guide.
