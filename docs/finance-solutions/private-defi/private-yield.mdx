...
:::info Testnet only
Private yield runs on testnet and is not available on production chains. The vault simulates yield by periodically adding assets to the pool rather than sourcing returns from a live lending market, and uses test tokens with no real-world value.
:::
...
The vault is an [ERC-4626](https://eips.ethereum.org/EIPS/eip-4626) contract whose internal balances are held as secret shares across the TACEO MPC network, exactly like private transfer balances. Neither the balance nor the yield accrual is ever written to the chain in plaintext.
...
:::tip Token support
The description below uses one token throughout for simplicity. The vault standard and private yield support any supported token.
:::
