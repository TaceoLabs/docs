...
**Client** — the agent or application making the API request. It holds the secret key to the spending wallet, generates
...
Required`. The response body contains a base64-encoded `PaymentRequired` payload advertising the
...
7. Re-send the original request with a `PAYMENT` header containing the encoded payload.
...
If all checks pass, the facilitator returns `{ valid: true }` to the resource server.
