...
sequenceDiagram
    ...
    F->>X: transferFrom()
    Note over X: verify Groth16 proof onchain via ClientTransferVerifier
    X-->>F: action_index
...
| Onchain proof verification (client proof) | ~800,000 gas |
| Onchain proof verification (server proof) | ~1,100,000 gas |
