...
```bash
curl -X POST https://faucet.merces1.taceo.io/claim/<YOUR_WALLET_ADDRESS>
```
...
const response = await fetchWithPayment(
  'https://x402-server.merces.taceo.io/api/protected',
  { method: 'GET' }
);
...
async fn main() -> eyre::Result<()> {
    ...
    let response = http_client
        .get("https://x402-server.merces.taceo.io/api/protected")
        .send()
        .await?;
