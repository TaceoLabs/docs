...
- Node.js 18+ (TypeScript) or Rust 1.90+ (Rust)
- A wallet private key (or generate one — instructions below)
...
```bash
npm install @taceolabs/taceo-merces1-x402-js
```
...
alloy = { version = "2", default-features = false, features = ["signers"] }
```
...
Replace `<YOUR_WALLET_ADDRESS>` with your Ethereum wallet address. If you don't have one yet,
...
println!("Private key: {}", signer.credential().to_bytes().to_string());
...
2. The `ConfidentialEvmScheme` generates a Groth16 ZK proof (~800ms) and constructs the signed
   payment payload.
