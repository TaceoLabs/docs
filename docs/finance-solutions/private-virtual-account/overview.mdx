...
Merces wraps an existing ERC-20 — USDC, USDT — into a private form. The token, the chain, the wallet and the RPC all stay the same. There is no bridge, no new consensus, no migration.

![Virtual Account](virtual-account.jpg)

## What the account holds

The balance is held as secret shares across the TACEO Network's MPC operators, with a commitment onchain. No single party, TACEO included, sees a plaintext balance or amount. The account owner sees their own balance; everyone else sees a commitment.
...
For organisations, the same account underpins batch payments for payroll and contractors, treasury movements, fiat on and off ramps, agent payments, and RWA tokenization.

## How it is integrated

The integration model is API and SDK. Stablecoin issuers, fintechs and payment infrastructure builders white-label Merces into their own products, either embedded via the client SDK or operated through an enterprise dashboard. End users never see TACEO.

## How it works - Components

![Merces Architecture](merces_architecture.png)
